"""
1、带有蓝色边框的格子表示起始状态；
2、带有金黄色边框的格子表示终止状态，终止状态可以不止一个；
3、黑色格子表示障碍格子，个体一般不能进入；
4、其他不同颜色的格子表示格子的即时奖励值不同，奖励值为0的格子颜色为灰色，奖励值为负值
颜色显示偏向于红色，值越小，红色越深；奖励值大于0的格子偏向于显示绿色，值越大，鲁塞越饱满；
5、个体使用黄色圆形来表示；
"""
...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, n_width: int = 10, # 水平方向上格子数量
                 n_height: int = 7,  # 竖直方向上格子数量
                 u_size=40,  # 每一个单位格子的绘制边长（单位为像素值）
                 default_reward: float = 0,  # 默认每一个格子的即时奖励值
                 default_type=0,  # 以及默认格子的类型。格子类型值为0时个体可进入格子，类型为1表示障碍，个体不能进入
                 windy=False):
...
def SimpleGridWorld():
    ...
    env.observation_space.n = env.n_width * env.n_height
    env.refresh_setting()
    return env
